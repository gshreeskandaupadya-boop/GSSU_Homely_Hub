...
const sendMail = async (options) => {
  ...
  const transporter = nodemailer.createTransport({
    host: process.env.MAILTRAP_SMTP_HOST || process.env.MAIL_SMTP_HOST || "sandbox.smtp.mailtrap.io",
    port: Number(process.env.MAILTRAP_SMTP_PORT || process.env.MAIL_SMTP_PORT || 2525),
    secure: false,
    auth: {
      user: process.env.MAILTRAP_SMTP_USER || process.env.MAIL_SMTP_USER || "",
      pass: process.env.MAILTRAP_SMTP_PASS || process.env.MAIL_SMTP_PASS || "",
    },
  });
